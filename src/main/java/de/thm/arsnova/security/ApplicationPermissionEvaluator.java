...
public class ApplicationPermissionEvaluator implements PermissionEvaluator {
	...
	public boolean hasPermission(
		...
		switch (targetType) {
			case "session":
				final Session targetSession = sessionRepository.getSessionFromKeyword(targetId.toString());
				return targetSession != null && hasSessionPermission(username, targetSession, permission.toString());
			case "content":
				final Content targetContent = contentRepository.getQuestion(targetId.toString());
				return targetContent != null && hasContentPermission(username, targetContent, permission.toString());
			case "comment":
				final Comment targetComment = commentRepository.getInterposedQuestion(targetId.toString());
	...
	private boolean hasContentPermission(
		...
		switch (permission) {
			case "read":
				return sessionRepository.getSessionFromId(targetContent.getSessionId()).isActive();
			case "create":
			case "owner":
			case "update":
			case "delete":
				final Session session = sessionRepository.getSessionFromId(targetContent.getSessionId());
	...
	private boolean hasCommentPermission(
		...
		switch (permission) {
			case "read":
				...
				final Session session = sessionRepository.getSessionFromKeyword(targetComment.getSessionId());

				return session != null && session.getCreator().equals(username);
			case "create":
				return sessionRepository.getSessionFromId(targetComment.getSessionId()).isActive();
			case "owner":
			case "update":
