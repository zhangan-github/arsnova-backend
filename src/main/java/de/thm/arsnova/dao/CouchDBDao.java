...
public class CouchDBDao implements IDatabaseDao, ApplicationEventPublisherAware {
	...
	public List<Session> getSessionsForUsername(String username, final int start, final int limit) {
		final NovaView view = new NovaView("session2/partial_by_sessiontype_creator_name");
		if (start > 0) {
			view.setSkip(start);
		}
		if (limit > 0) {
	...
	public List<Session> getPublicPoolSessions() {
		...
		final NovaView view = new NovaView("session2/partial_by_ppsubject_name_for_publicpool");
	...
	public Session getSessionFromKeyword(final String keyword) {
		final NovaView view = new NovaView("session2/by_keyword");
	...
	public boolean sessionKeyAvailable(final String keyword) {
		final View view = new View("session2/by_keyword");
		final ViewResults results = getDatabase().view(view);
	...
	public List<Session> getCourseSessions(final List<Course> courses) {
		final ExtendedView view = new ExtendedView("session2/by_courseid");
		view.setIncludeDocs(true);
	...
	public int[] deleteInactiveGuestSessions(long lastActivityBefore) {
		NovaView view = new NovaView("session2/rev_by_lastactivity_for_guests");
	...
	public List<Motd> getAdminMotds() {
		final NovaView view = new NovaView("motd2/doc_by_audience_for_global");
		view.setKey("admin");
	...
	public List<Motd> getMotdsForTutors() {
		final NovaView view1 = new NovaView("motd2/doc_by_audience_for_global");
		final NovaView view2 = new NovaView("motd2/doc_by_audience_for_global");
	...
	public List<Motd> getMotdsForSession(final String sessionkey) {
		final NovaView view = new NovaView("motd2/doc_by_sessionkey");
	...
	public Motd getMotdByKey(String key) {
		final NovaView view = new NovaView("motd2/by_motdkey");
		view.setIncludeDocs(true);
		view.setKey(key);
		Motd motd = new Motd();
